...
library(stringr)
witnesses <- person %>%
  filter(address_street_name %in% c("Northwestern Dr", "Franklin Ave") & (address_number == max(address_number) | str_detect(name,"Annabel")))

witness.interviews <- left_join(witnesses, interview, by = c("id" = "person_id"))
witness.interviews$transcript

annabel.checkin <- get_fit_now_member %>%
  inner_join(witnesses, by = c("person_id" = "id")) %>%
  semi_join(get_fit_now_check_in, ., by = c("membership_id" = "id"))

testimonysuspects <- get_fit_now_member %>%
  filter(membership_status %in% c("gold"), str_detect(id, "^48Z"))

driverslmerge <- left_join(testimonysuspects, person, by = c("person_id" = "id"))

licenseplate <- left_join(driverslmerge, drivers_license, by = c("license_id" = "id"))

suspect_check_in <- left_join(licenseplate, get_fit_now_check_in, by = c("id" = "membership_id"))

suspect_testimony <- left_join(suspect_check_in, interview, by = c("person_id" = "person_id"))
suspect_testimony$transcript

facebook_event_checkin %>%
  filter(event_name %in% c("SQL Symphony Concert"))

redhead_suspect <- drivers_license%>%
  filter(hair_color %in% c("red") & gender %in% c("female") & car_make %in% c("Tesla") & str_detect(car_model, "S"))

redhead_suspect <- left_join(redhead_suspect, person, by = c("id" = "license_id"))

final_suspect <- inner_join(redhead_suspect, facebook_event_checkin, by = c("id.y" = "person_id"))
```
